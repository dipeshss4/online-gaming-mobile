...
export function NativeRoulette({game,token,userId,initialBalance,onClose,onSettled}:{game:Game;token:string;userId:string;initialBalance:Balance|null;onClose:()=>void;onSettled:()=>void}) {
  ...
  async function spin(){
    ...
    try{
      ...
      if(!alive.current)return;
      await clearPending(userId);setPending(null);setResult(data);setWallet({...wallet,balance:data.balance,currency:data.currency});setTicket({});onSettled();
  ...
  const button=(key:string)=><Pressable key={key} accessibilityRole="button" accessibilityLabel={`Bet ${label(key)}`} disabled={busy||!!pending} onPress={()=>add(key)} style={[r.square,{backgroundColor:key.startsWith('number-')?color(Number(key.slice(7))):'#144837'},!!ticket[key]&&{borderColor:'#ffe3a1',borderWidth:2}]}><Text style={r.number}>{label(key)}</Text>{!!ticket[key]&&<Text style={r.chipValue}>{(ticket[key]/100).toFixed(2)}</Text>}</Pressable>;
  return <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
    <Pressable accessibilityRole="button" disabled={busy} onPress={onClose}><Text style={s.link}>‹ Back to games</Text></Pressable><Text style={s.kicker}>EUROPEAN COLLECTION · SINGLE ZERO</Text><Text style={s.title}>{game.name}</Text>
    <LinearGradient colors={['#2b1915','#111117']} style={r.cabinet}><Text style={r.pointer}>▼</Text><Animated.View style={[r.wheel,{transform:[{rotate:rotation.interpolate({inputRange:[0,1800],outputRange:['0deg','1800deg']})}]}]}>{order.map((n,i)=>{const angle=i*2*Math.PI/37;return <View key={n} style={[r.pocket,{left:118+108*Math.sin(angle)-9,top:118-108*Math.cos(angle)-12,backgroundColor:color(n),transform:[{rotate:`${i*360/37}deg`}]}]}><Text style={{color:'#fff',fontSize:9}}>{n}</Text></View>;})}<View style={r.hub}><Text style={{color:'#e7c888',fontSize:42}}>✦</Text></View></Animated.View><Text accessibilityLiveRegion="polite" style={[s.title,{textAlign:'center'}]}>{busy?'Wheel in motion…':result?`Result: ${result.symbols[0]} ${result.symbols[1]}`:'Place your chips'}</Text></LinearGradient>
    ...
    <Text style={s.kicker}>CHIP VALUE · TAP A SELECTION TO ADD</Text><View style={r.chips}>{[...new Set([game.minStake,1,5,10,25])].filter(n=>n>=game.minStake&&n<=game.maxStake).map(n=><Pressable key={n} accessibilityRole="button" accessibilityLabel={`Chip ${n}`} disabled={busy||!!pending} onPress={()=>setChip(Math.round(n*100))} style={[r.chip,chip===Math.round(n*100)&&{borderColor:'#ffe3a1',backgroundColor:'#654a24'}]}><Text style={s.accent}>{n}</Text></Pressable>)}</View>
    ...
    <Text style={s.kicker}>YOUR TICKET</Text>{Object.entries(ticket).map(([key,cents])=><Pressable key={key} accessibilityRole="button" accessibilityLabel={`Remove ${label(key)}`} disabled={busy||!!pending} onPress={()=>setTicket(prev=>Object.fromEntries(Object.entries(prev).filter(([k])=>k!==key)))}><Text style={s.muted}>{label(key)} · {(cents/100).toFixed(2)}    × Remove</Text></Pressable>)}
    <Pressable accessibilityRole="button" disabled={busy||!!pending} onPress={()=>setTicket({})}><Text style={s.link}>Clear ticket</Text></Pressable><Pressable accessibilityRole="button" disabled={busy||!ready||(!!pending&&pending.gameCode!==game.code)} onPress={spin} style={[s.button,(busy||!ready)&&{opacity:.5}]}><Text style={s.buttonText}>{busy?'Spinning…':pending?'Recover ticket':'SPIN ROULETTE'}</Text></Pressable>
...
const r=StyleSheet.create({cabinet:{borderWidth:1,borderColor:'#a58b50',borderRadius:18,padding:16,gap:12,alignItems:'center'},wheel:{width:240,height:240,borderWidth:2,borderColor:'#e7c888',borderRadius:120,backgroundColor:'#39291d'},pocket:{position:'absolute',width:18,height:24,alignItems:'center',justifyContent:'center'},hub:{position:'absolute',left:60,top:60,width:116,height:116,borderRadius:60,borderWidth:3,borderColor:'#bd9253',alignItems:'center',justifyContent:'center',backgroundColor:'#6b4825'},pointer:{color:'#ffe3a1',fontSize:20,marginBottom:-14,zIndex:2},chips:{flexDirection:'row',gap:12,flexWrap:'wrap'},chip:{width:48,height:48,borderRadius:24,borderWidth:2,borderColor:'#7d6741',alignItems:'center',justifyContent:'center'},table:{backgroundColor:'#103629',padding:10,borderWidth:1,borderColor:'#b99a61',borderRadius:14,gap:8},grid:{flexDirection:'row',flexWrap:'wrap',gap:5},square:{width:'31%',flexGrow:1,minHeight:48,borderWidth:1,borderColor:'#bfa77766',alignItems:'center',justifyContent:'center',borderRadius:4},number:{color:'#fff1d5',fontSize:14,fontWeight:'700'},chipValue:{color:'#ffe3a1',fontSize:10}});
