 ...
 */
export function LandscapeGame({ children, stageItems = 4, stage = 0.68 }: {
  children: React.ReactNode; stageItems?: number; stage?: number;
}) {
  const { width, height } = useWindowDimensions();
  const items = React.Children.toArray(children);
  if (width <= height) return <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">{children}</ScrollView>;
  ...
  return <View style={{ flex: 1, flexDirection: 'row', backgroundColor: '#111117' }}>
    <ScrollView style={{ width: width - controls }} contentContainerStyle={[s.content, { padding: 12, gap: 8 }]} keyboardShouldPersistTaps="handled">{items.slice(0, stageItems)}</ScrollView>
